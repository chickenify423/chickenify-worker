...
MODEL_CHECKPOINT = os.getenv("MODEL_CHECKPOINT")  # optional path to trained model
...
async def infer(
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # Normalize to mono WAV for processing
        subprocess.run(
            ["ffmpeg", "-y", "-i", in_any, "-ar", "44100", "-ac", "1", in_wav],
            check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )

        # 1) Demucs separation (vocals + accompaniment)
        subprocess.run(["demucs", "-n", "htdemucs", "-o", td, in_wav], check=True)
